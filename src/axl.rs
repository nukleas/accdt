...
const SECTION_NAMES: &[&str] = &["FormHeader", "FormFooter", "Detail", "PageHeader", "PageFooter", "ReportHeader", "ReportFooter", "GroupHeader", "GroupFooter"];

fn children<'a, 'b>(n: XmlNode<'a, 'b>, tag: &str) -> Vec<XmlNode<'a, 'b>> {
    n.children().filter(|c| c.is_element() && c.tag_name().name() == tag).collect()
}
...
fn set(node: &mut Node, key: &str, value: &str) {
    ...
    node.properties.entry(key.to_string()).or_insert_with(|| value.to_string());
}
...
pub(crate) fn form_document(part: &str, xml: &str) -> crate::Result<Document> {
    ...
    let mut root = Node { kind: "Form".into(), ..Default::default() };
    let view = doc.root_element();
    if let Some(rs) = view.descendants().find(|n| n.is_element() && n.tag_name().name() == "RecordSource").and_then(|n| n.text()) {
        set(&mut root, "RecordSource", rs.trim());
    }
    if let Some(form) = view.descendants().find(|n| n.is_element() && n.tag_name().name() == "Form") {
    ...
    for child in parent.children().filter(|c| c.is_element()) {
        ...
        match attr(child, "Name") {
            Some(name) if !name.is_empty() => {
                let kind = if SECTION_NAMES.contains(&name) && tag == "Section" { name.to_string() } else { tag.to_string() };
...
fn attach_macros(view: XmlNode, root: &mut Node) {
    for m in view.descendants().filter(|n| n.is_element() && n.tag_name().name() == "UserInterfaceMacro") {
        let target = attr(m, "For").unwrap_or("");
        let event = attr(m, "Event").unwrap_or("OnClick");
        let event = if event.starts_with("On") || event.starts_with("After") || event.starts_with("Before") { event.to_string() } else { format!("On{event}") };
        let mut em = Node { kind: format!("{event}EmMacro"), ..Default::default() };
        set(&mut em, "Version", "axl");
        flatten_actions(m, None, &mut em.children);
        let owner = if target.is_empty() { Some(&mut *root) } else { find_control(root, target) };
...
pub(crate) fn flatten_actions(n: XmlNode, condition: Option<&str>, out: &mut Vec<Node>) {
    for child in n.children().filter(|c| c.is_element()) {
        match child.tag_name().name() {
            "Action" => {
                let mut block = Node { kind: "Block".into(), ..Default::default() };
                if let Some(c) = condition {
                    set(&mut block, "Condition", c);
                }
                set(&mut block, "Action", attr(child, "Name").unwrap_or(""));
                for arg in child.children().filter(|c| c.is_element() && c.tag_name().name() == "Argument") {
            ...
            "If" | "ElseIf" => {
                let cond = child.children().find(|c| c.is_element() && c.tag_name().name() == "Condition").and_then(|c| c.text()).map(str::trim);
                flatten_actions(child, cond.or(condition), out);
            }
            "Else" => flatten_actions(child, Some("Else"), out),
            "Sub" => {
                let mut block = Node { kind: "Block".into(), ..Default::default() };
                set(&mut block, "Comment", &format!("Submacro {}", attr(child, "Name").unwrap_or("")));
                out.push(block);
...
pub(crate) fn report_document(part: &str, xml: &str) -> crate::Result<Document> {
    let doc = text::parse_xml(part, xml)?;
    let mut root = Node { kind: "Report".into(), ..Default::default() };
    let report = doc.root_element();
    if let Some(cmd) = report.descendants().find(|n| n.is_element() && n.tag_name().name() == "CommandText").and_then(|n| n.text()) {
        set(&mut root, "RecordSource", cmd.trim());
    }
    for group in report.descendants().filter(|n| n.is_element() && n.tag_name().name() == "GroupExpression") {
...
fn collect_report_items(parent: XmlNode, out: &mut Vec<Node>) {
    for child in parent.children().filter(|c| c.is_element()) {
        let tag = child.tag_name().name();
        match attr(child, "Name") {
            Some(name) if !name.is_empty() && !matches!(tag, "Field" | "DataSet" | "DataSource" | "CustomProperty") => {
                let kind = if SECTION_NAMES.contains(&name) && tag == "Rectangle" { name.to_string() } else { tag.to_string() };
                let mut node = Node { kind, ..Default::default() };
                set(&mut node, "Name", name);
                for prop in child.children().filter(|c| c.is_element()) {
                    let pname = prop.tag_name().name();
                    if let Some(t) = prop.text().filter(|t| !t.trim().is_empty()).filter(|_| matches!(pname, "Height" | "Width" | "Top" | "Left" | "DataSetName" | "Hidden" | "CanGrow")) {
                        set(&mut node, pname, t.trim());
                    }
                }
                if let Some(v) = child.descendants().find(|d| d.is_element() && d.tag_name().name() == "Value").and_then(|d| d.text()) {
                    let v = v.trim();
                    set(&mut node, if v.starts_with('=') { "ControlSource" } else { "Caption" }, v);
                }
                let mut kids = Vec::new();
...
pub(crate) fn query_def(part: &str, xml: &str) -> crate::Result<QueryDef> {
    ...
    let mut def = QueryDef { operation: Some(crate::query::Operation::Select), ..Default::default() };
    let section = |tag: &str| q.children().find(|c| c.is_element() && c.tag_name().name() == tag);
    if let Some(refs) = section("References") {
        for r in children(refs, "Reference") {
            def.tables.push((attr(r, "Source").unwrap_or("").to_string(), attr(r, "Alias").map(String::from)));
        }
    }
    if let Some(joins) = section("Joins") {
        for j in children(joins, "Join") {
            def.joins.push(Join {
                left_table: attr(j, "LeftSource").or(attr(j, "Left")).unwrap_or("").to_string(),
                right_table: attr(j, "RightSource").or(attr(j, "Right")).unwrap_or("").to_string(),
                expression: j.descendants().find(|d| d.is_element() && d.tag_name().name() == "Expression").and_then(|d| d.text()).unwrap_or("").trim().to_string(),
    ...
    if let Some(results) = section("Results") {
        for p in children(results, "Property") {
            ...
            let expr = p.children().find(|c| c.is_element() && c.tag_name().name() == "Expression").and_then(|c| c.text()).map(|t| t.trim().to_string());
            ...
            def.columns.push(OutputColumn { expression, alias: attr(p, "Alias").map(String::from) });
        }
    }
    if let Some(r) = section("Restriction") {
        def.where_clause = r.descendants().find(|d| d.is_element() && d.tag_name().name() == "Expression").and_then(|d| d.text()).map(|t| t.trim().to_string());
    }
    if let Some(o) = section("Ordering") {
        for ord in children(o, "Order") {
            ...
            def.order_by.push((expr, attr(ord, "Direction").is_some_and(|d| d.eq_ignore_ascii_case("Descending"))));
        }
    }
...
pub(crate) fn list_definition(part: &str, bytes: &[u8]) -> crate::Result<ListDefinition> {
    ...
    let find_text = |tag: &str| root.descendants().find(|n| n.is_element() && n.tag_name().name() == tag).and_then(|n| n.text()).map(|t| t.trim().to_string());
    let mut def = ListDefinition {
        part: part.to_string(),
        list_name: root
            .descendants()
            .find(|n| n.is_element() && n.tag_name().name() == "AddList")
            .and_then(|a| a.children().find(|c| c.is_element() && c.tag_name().name() == "listName"))
            .and_then(|n| n.text())
    ...
    for group in root.descendants().filter(|n| n.is_element() && matches!(n.tag_name().name(), "newFields" | "updateFields")) {
        let added = group.tag_name().name() == "newFields";
        for f in group.descendants().filter(|n| n.is_element() && n.tag_name().name() == "Field") {
...
mod tests {
    ...
    fn form_becomes_a_design() {
        ...
        let d = Design { name: "Contacts".into(), kind: DesignKind::Form, document: doc, text: FORM.into() };
        ...
        let events = d.events();
        assert!(events.iter().any(|e| e.owner == "cmdSearch" && e.event == "Click" && e.value == "[Embedded Macro]"), "{events:?}");
        let m = &d.embedded_macros()["cmdSearch.Click"];
        assert_eq!(m.actions[0].action, "SetFilter");
        assert_eq!(m.actions[0].condition.as_deref(), Some("=txtSearch=\"Search...\""));
    }
    ...
    fn query_becomes_a_def() {
        ...
        let sql = def.to_sql();
        assert_eq!(sql.text, "SELECT Contacts.*, =Concatenate.Db(LastName,FirstName) AS Searchable\nFROM Contacts\nORDER BY Contacts.ContactName DESC\n;");
    }
