...
pub(crate) fn parse(part: &str, bytes: &[u8]) -> crate::Result<Vec<DataMacro>> {
    ...
    let mut out = Vec::new();
    for dm in doc.root_element().children().filter(|n| n.is_element() && n.tag_name().name() == "DataMacro") {
        let event = dm.attribute("Event").or_else(|| dm.attribute("Name")).unwrap_or("").to_string();
        let actions = dm
            .descendants()
            ...
            .map(|a| DataMacroAction {
                ...
                arguments: a
                    ...
                    .map(|c| (c.attribute("Name").unwrap_or("").to_string(), c.text().unwrap_or("").trim().to_string()))
                    .collect(),
            })
            .collect();
        let range = dm.range();
        out.push(DataMacro { event, actions, xml: xml[range].to_string() });
    }
    Ok(out)
}
