...
fn wss_tasks_template_links_sharepoint_lists() {
    let Some(pkg) = open("wss-107-tasks.accdt") else { return };
    assert_eq!(pkg.objects_of(ObjectKind::Table).count(), 2);
    let tasks = pkg.table("Tasks").unwrap();
    let sp = tasks.sharepoint.as_ref().expect("Tasks is a SharePoint list link");
    ...
    assert!(sp.default_view_url.as_deref().is_some_and(|u| u.ends_with("detail.aspx")));
    assert!(pkg.forms().unwrap().iter().all(|f| !f.is_axl() && !f.controls().is_empty()));
}

#[test]
fn contacts_web_database_reads_axl_objects() {
    let Some(pkg) = open("access2010-contacts-web.accdt") else { return };
    ...
    assert!(!names.iter().any(|n| n.contains("_Flip") || n.contains("_AddFurigana") || n.starts_with("ooo")), "{names:?}");
    ...
    assert!(ctrls.iter().any(|c| c.name == "txtContactName" && c.control_type == "TextBox" && c.get("ControlSource") == Some("ContactName")), "{:?}", ctrls.iter().map(|c| &c.name).collect::<Vec<_>>());
    ...
    assert!(pkg.form_variation("NameCard", "FlipAddress").unwrap().controls().len() > 1);

    // AXL reports and queries.
    let report = pkg.report("Comments").unwrap();
    assert_eq!(report.record_source(), Some("Comments"));
    assert!(report.controls().iter().any(|c| c.control_type == "Textbox" || c.is_section()));
    let q = pkg.query("ContactsExtended").unwrap();
    let sql = q.to_sql();
    assert_eq!(sql.source, SqlSource::Reconstructed);
    assert!(sql.text.contains("Contacts.*") && sql.text.contains("AS Searchable"), "{}", sql.text);
    ...
    assert!(comments.fields.iter().any(|f| f.display_name == "CommentDate" && f.field_type == "DateTime" && f.added));
