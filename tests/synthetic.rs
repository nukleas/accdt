...
use accdt::{JetType, ObjectKind, Operation, Package};
...
fn build() -> Vec<u8> {
    ...
    add("template/database/databaseProperties.xml", &utf16("<?xml version=\"1.0\" encoding=\"UTF-16\" standalone=\"no\"?><Application xmlns=\"x\"><Properties><Property Name=\"AccessVersion\" Type=\"10\">09.50</Property><Property Name=\"StartUpForm\" Type=\"10\">frmMain</Property></Properties></Application>"));
    ...
    add("template/database/objects/sampleData/tableCompanies.xml", b"<root><dataroot><Companies><ID>1</ID><Company_x0020_Name>Acme &amp; Co</Company_x0020_Name><Active>1</Active></Companies><Companies><ID>2</ID><Active>0</Active></Companies></dataroot></root>");
    add("template/database/objects/properties/tableCompanies_Metadata.xml", b"<AccessObject><Type>Table</Type><Name>Companies</Name></AccessObject>");
    ...
    add("template/database/objects/dataMacros/datamacrosCompanies.axl", &utf16("<?xml version=\"1.0\" encoding=\"UTF-16\" standalone=\"no\"?><DataMacros xmlns=\"a\"><DataMacro Event=\"BeforeChange\"><Statements><Action Name=\"SetField\"><Argument Name=\"Field\">Active</Argument><Argument Name=\"Value\">1</Argument></Action></Statements></DataMacro></DataMacros>"));
    // Form named with a space, real name from metadata.
    add("template/database/objects/formfrmMain.txt", &utf16("Version =21\r\nBegin Form\r\n    RecordSource =\"Companies\"\r\n    Caption =\"Main\"\r\n    Begin\r\n        Begin Section\r\n            Name =\"Detail\"\r\n            Begin\r\n                Begin CommandButton\r\n                    Name =\"cmdGo\"\r\n                    OnClick =\"[Event Procedure]\"\r\n                End\r\n            End\r\n        End\r\n    End\r\nEnd\r\nCodeBehindForm\r\nPrivate Sub cmdGo_Click()\r\nEnd Sub\r\n"));
    add("template/database/objects/properties/formfrmMain_Metadata.xml", b"<AccessObject><Type>Form</Type><Name>Main Form</Name></AccessObject>");
    add("template/database/objects/modulemodUtil.txt", b"\xEF\xBB\xBFOption Explicit\r\nPublic Function Hi() As String\r\n    Hi = \"hi\"\r\nEnd Function\r\n");
    add("template/database/objects/properties/modulemodUtil_Metadata.xml", b"<AccessObject><Type>Module</Type><Name>modUtil</Name></AccessObject>");
    add("template/database/objects/macroAutoExec.txt", &utf16("Version =196611\r\nBegin\r\n    Action =\"OpenForm\"\r\n    Argument =\"Main Form\"\r\nEnd\r\n"));
...
fn reads_every_part() {
    ...
    assert_eq!(pkg.core_properties().unwrap().title.as_deref(), Some("Demo"));
    assert_eq!(pkg.template().unwrap().required_access_version.as_deref(), Some("14"));
    let props = pkg.database_properties().unwrap();
    assert!(props.iter().any(|p| p.name == "StartUpForm" && p.value == "frmMain"));

    let kinds: Vec<(ObjectKind, &str)> = pkg.objects().iter().map(|o| (o.kind, o.name.as_str())).collect();
    assert_eq!(kinds, vec![(ObjectKind::Table, "Companies"), (ObjectKind::Query, "qryActive"), (ObjectKind::Form, "Main Form"), (ObjectKind::Macro, "AutoExec"), (ObjectKind::Module, "modUtil")]);
    ...
    assert_eq!(t.rows, vec![vec![Some("1".into()), Some("Acme & Co".into()), Some("1".into())], vec![Some("2".into()), None, Some("0".into())]]);
    assert_eq!(t.to_csv(), "\"ID\",\"Company Name\",\"Active\"\n\"1\",\"Acme & Co\",\"1\"\n\"2\",,\"0\"\n");
    let dm = pkg.data_macros(pkg.object(ObjectKind::Table, "companies").unwrap()).unwrap();
    assert_eq!(dm[0].event, "BeforeChange");
    assert_eq!(dm[0].actions[0].arguments[0], ("Field".to_string(), "Active".to_string()));

    let f = pkg.form("Main Form").unwrap().unwrap();
    assert_eq!(f.record_source(), Some("Companies"));
    assert_eq!(f.controls().iter().map(|c| c.name.clone()).collect::<Vec<_>>(), vec!["Detail", "cmdGo"]);
    assert_eq!(f.events()[0].procedure_name().as_deref(), Some("cmdGo_Click"));
    assert!(f.code_behind.unwrap().contains("cmdGo_Click"));

    assert_eq!(pkg.modules().unwrap()[0].source.lines().next(), Some("Option Explicit"));
    let m = &pkg.macros().unwrap()[0];
    assert_eq!(m.actions[0].arguments, vec!["Main Form"]);
    let q = &pkg.queries().unwrap()[0];
    assert_eq!(q.definition.operation, Some(Operation::Select));
    assert_eq!(q.to_sql().0, "SELECT Companies.ID\nFROM Companies\nWHERE Companies.Active=True\n;");

    let rels = pkg.relationships().unwrap();
    assert_eq!(rels[0].columns, vec![("CompanyID".to_string(), "ID".to_string())]);
    ...
    let res = pkg.resources();
    assert_eq!(res[0].name, "logo");
    assert!(pkg.part("template/database/objects/tableCompanies.xsd").is_some());
}
...
fn rejects_non_packages() {
    ...
    assert!(matches!(Package::from_bytes(&bytes), Err(accdt::Error::NotAPackage(_))));
}
