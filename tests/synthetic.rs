...
fn reads_every_part() {
    ...
    assert_eq!(t.columns.len(), 4, "the Files column only exists in the data");
    ...
    assert_eq!(t.rows[1], vec![Some(Value::Text("2".into())), None, Some(Value::Text("0".into())), None]);
    assert_eq!(t.to_csv(), "\"ID\",\"Company Name\",\"Active\",\"Files\"\n\"1\",\"Acme & Co\",\"1\",\"FileData=<4 base64 chars>; FileName=a.png | FileData=<4 base64 chars>; FileName=b.png\"\n\"2\",,\"0\",\n");
    ...
    assert!(matches!(pkg.form("nope"), Err(accdt::Error::MissingObject { kind: "form", .. })));

    assert_eq!(pkg.module("modUtil").unwrap().source.lines().next(), Some("Option Explicit"));
    ...
    assert_eq!(sql.text, "SELECT Companies.ID\nFROM Companies\nWHERE Companies.Active=True\n;");
    ...
    assert_eq!(pkg.template().unwrap().template_type.as_deref(), Some("Getting Started"));
...
fn malformed_rels_are_errors_not_silence() {
    ...
    add("template/database/objects/_rels/tableT.xsd.rels", b"<Relationships><Broken");
    let bytes = zip.finish().unwrap().into_inner();
    assert!(matches!(Package::from_bytes(&bytes), Err(accdt::Error::Xml { .. })));
}
