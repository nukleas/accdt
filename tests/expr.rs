...
fn projects_filters_rows() {
    ...
    let strings: Vec<&str> = t
        .rows
        .iter()
        .filter_map(|r| r.cells[col].value().and_then(Value::as_str))
        .collect();
...
fn projects_project_details_control_sources() {
    ...
    let sqlite_of = |raw: &str| {
        f.controls()
            .iter()
            .find(|c| c.raw_property("ControlSource") == Some(raw))
...
fn projects_employee_phone_list_and_tasks_subreport() {
    ...
    let letter = phone
        .controls()
        .iter()
        .find(|c| c.raw_property("ControlSource") == Some("=UCase(Left(Nz([File As]),1))"))
    ...
    let cost = sub
        .controls()
        .iter()
        .find(|c| c.raw_property("ControlSource") == Some("=Nz(Sum([Cost]),0)"))
        .expect("Nz Sum Cost")
        .control_source_expr()
        .unwrap()
        .unwrap()
